...
import { ArrowLeft, User, GraduationCap, Activity, Award, Image, Edit, Save } from 'lucide-react';
...
const AthleteProfile = ({ isMyProfile = false }) => {
  ...
  const effectiveId = isMyProfile ? user?._id : id;

  // Admins cannot access athlete profiles
  if (user?.role === 'admin') {
...
const tabs = [
      ...
      ...(['coach', 'sport_coordinator', 'admin'].includes(user?.role) ? [{ id: 'performance', label: 'Performance', icon: Award }] : []),
  ...
  if (!athlete) {
    if (isMyProfile && user?.role === 'athlete') {
      return (
        <Navigate to="/athletes/add" replace />
      );
    }
    return (
      <div className="p-6">
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        {isMyProfile && athlete && athlete._id ? (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card>
            ...
            {activeTab === 'academic' && athlete.academic?.adviser && (
