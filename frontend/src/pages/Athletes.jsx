...
import { Users, Plus, Search, Filter, Eye, Edit, Trash2, ChevronLeft, ChevronRight, UserPlus } from 'lucide-react';
...
const Athletes = () => {
  ...
  const [assignTeamModal, setAssignTeamModal] = useState({ open: false, athleteId: null });
...
useEffect(() => {
  ...
  const handleAssignTeam = (athleteId) => {
    setAssignTeamModal({ open: true, athleteId });
    fetchTeams();
  };

  const assignToTeam = async () => {
    if (!selectedTeamId || !assignTeamModal.athleteId) return;
    setAssignLoading(true);
    try {
      await api.post(`/teams/${selectedTeamId}/roster`, {
        athleteId: assignTeamModal.athleteId
      });
      setAssignTeamModal({ open: false, athleteId: null });
      setSelectedTeamId('');
    } catch (error) {
      console.error('Failed to assign athlete to team:', error);
    } finally {
...
<td className="py-3 px-4">
                         <div className="flex items-center justify-end gap-2">
                           ...
                           <button
                             onClick={() => handleAssignTeam(athlete._id)}
